...
## Setting up a provider in a module

When you create a terraform module, you may be tempted to add a `_settings.tf` file with a `provider` block in it. Using this pattern, you will have a structure looking like this.
...
On top of that, declaring the provider in each module (or hand-writing a `_settings.tf` per layer) drifts over time: version constraints and provider configuration end up inconsistent across the repository.

### Do this instead

Define the provider **once**, centrally, in the Terragrunt `root.hcl` with a `generate` block. Terragrunt renders it into a `_settings.tf` file in every layer, so the whole IaC repository shares the exact same provider and version configuration. Modules never declare a provider — they only use the one injected by the layer.
...
See the [Terragrunt context pattern](../terragrunt/context_pattern.md#roothcl) for the full `root.hcl` structure, and the [providers within modules](https://developer.hashicorp.com/terraform/language/modules/develop/providers) documentation.
